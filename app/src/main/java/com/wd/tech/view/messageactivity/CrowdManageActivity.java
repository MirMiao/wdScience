...
import com.wd.tech.adapter.infromation.Mycrowdmanageadapter;
import com.wd.tech.adapter.infromation.Mycrowdmemberadapter;
import com.wd.tech.adapter.infromation.Mycrowdzhuadapter;
...
public class CrowdManageActivity extends BaseActivity<Presenter> implements IContract.IView, View.OnClickListener {
    ...
    @BindView(R.id.crowdManagzhuRecyclerView)
    SwipeRecyclerView crowdManagzhuRecyclerView;
    @BindView(R.id.crowdManagageRecyclerView)
    SwipeRecyclerView crowdManagageRecyclerView;
    @BindView(R.id.btn_crowd_memberRecyclerView)
    SwipeRecyclerView btnCrowdMemberRecyclerView;
    private int groupIdmanage;
    private List<CrowdGroupAllUserMessageBean.ResultBean> crowdgroupallusermessageresult;
    private PopupWindow popupWindow;
    private int madapterPosition;
    private Mycrowdzhuadapter mycrowdzhuadapter;
    private Mycrowdmanageadapter mycrowdmanageadapter;
    private Mycrowdmemberadapter mycrowdmemberadapter;
    ...
    protected void initData() {
        ...
        // 设置监听器。
        crowdManagageRecyclerView.setSwipeItemMenuEnabled(true);
        crowdManagageRecyclerView.setSwipeMenuCreator(mSwipeMenuCreatorguan);
        // 菜单点击监听。
        crowdManagageRecyclerView.setOnItemMenuClickListener(mMenuItemClickListener);

        btnCrowdMemberRecyclerView.setSwipeItemMenuEnabled(true);
        btnCrowdMemberRecyclerView.setSwipeMenuCreator(mSwipeMenuCreatoryuan);
        // 菜单点击监听。
        btnCrowdMemberRecyclerView.setOnItemMenuClickListener(mMenuItemClickListener);
    }
    ...
    // 创建菜单：
    private SwipeMenuCreator mSwipeMenuCreatorguan = new SwipeMenuCreator() {
        @Override
    ...
    private SwipeMenuCreator mSwipeMenuCreatoryuan = new SwipeMenuCreator() {
        @Override
        public void onCreateMenu(SwipeMenu leftMenu, SwipeMenu rightMenu, int viewType) {
    ...
    public void success(Object o) {
        if (o instanceof CrowdGroupAllUserMessageBean) {
            ...
            mycrowdzhuadapter = new Mycrowdzhuadapter(crowdgroupallusermessageresult,this);
            mycrowdmanageadapter = new Mycrowdmanageadapter(crowdgroupallusermessageresult, this);
            mycrowdmemberadapter = new Mycrowdmemberadapter(crowdgroupallusermessageresult, this);
            for (int i = 0; i < crowdgroupallusermessageresult.size(); i++) {
                if (crowdgroupallusermessageresult.get(i).getRole()==3){
                    crowdManagzhuRecyclerView.setAdapter(mycrowdzhuadapter);
                    crowdManagzhuRecyclerView.setLayoutManager(new LinearLayoutManager(this));
                }
                if (crowdgroupallusermessageresult.get(i).getRole()==2){
                    crowdManagageRecyclerView.setAdapter(mycrowdmanageadapter);
                    crowdManagageRecyclerView.setLayoutManager(new LinearLayoutManager(this));
                }
                if (crowdgroupallusermessageresult.get(i).getRole()==1){
                    btnCrowdMemberRecyclerView.setAdapter(mycrowdmemberadapter);
                    btnCrowdMemberRecyclerView.setLayoutManager(new LinearLayoutManager(this));
                }
            }
        }
        //撤销
        if (o instanceof AdjustCrowdMemberBean) {
            if (((AdjustCrowdMemberBean) o).getStatus().equals("0000")) {
                ...
                presenter.getCrowdGroupAllUserMessageBeandata(groupIdmanage);
                mycrowdmanageadapter.notifyDataSetChanged();
                mycrowdmemberadapter.notifyDataSetChanged();
            }
        }
        //移除
